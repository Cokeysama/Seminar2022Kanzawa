...
int main(void){
  std::string filenameData("IrisRaw.dat");
#ifdef CHECK_ANSWER
  ...
  Vector indicator(test.data_number(), 0.0, "all");
  for(int k=0;k<test.data_number();k++){
    for(int i=0;i<test.centers_number();i++){
      if(test.crispMembership()[i][k]==1.0){
	indicator[k]=i;
      }
    }
  }
  std::cout << "u=" << indicator << std::endl;
  ...
  Vector correctIndicator(test.data_number(), 0.0, "all");
  for(int k=0;k<test.data_number();k++){
    for(int i=0;i<test.centers_number();i++){
      if(test.correctCrispMembership()[i][k]==1.0){
	correctIndicator[k]=i;
      }
    }
  }
  std::cout << "w=" << correctIndicator << std::endl;
